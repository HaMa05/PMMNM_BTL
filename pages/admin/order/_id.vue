      ...
      <div class="cart-wrapper">
        ...
        <div class="clearfix">
          <div
            ...
            v-for="item in billDetail.bill_detail"
            :key="item.id"
          >
            <div class="image">
              <a href="product.html"
            ...
            <div class="price">
              <span class="final h3">{{
                Intl.NumberFormat("vi-VN", {
                  style: "currency",
                  currency: "VND"
                }).format((item.unit_price * (100 - item.sale)) / 100)
              }}</span>
...
export default {
  ...
  created() {
    ...
    this.$axios.get(`/admin/bill/${id}`).then(res => {
      ...
      this.billDetail.order_date = moment(
        String(this.billDetail.created_at)
      ).format("DD/MM/YYYY");
      var product = res.data.bill_detail;
      console.log(product);
      product.forEach(element => {
        this.total +=
          ((element.unit_price * (100 - element.sale)) / 100) *
          element.quantity;
      });
    });
  }
